...
/*!
	\brief	Create multiple paths from start to end in C++, with a cost type. 
			Both out_ members return as arrays of pointers,
			with one for each element. 
			Sizes of zero represent non-existent paths, and will be set to null pointers
	...
	\param	end					An array of ids for ending nodes.
								Length must match that of start and all the IDS must belong to
								nodes that already exist within the graph.
	\param	out_path_ptr_holder	Pointer to a buffer of (Path *)
	\param	out_path_member_ptr_holder	Pointer to a buffer of (PathMember *)
	\param	out_sizes			The (PathMember *) count, how many PathMember objects compose a Path
	\param	num_paths			The maximum (Path *) count, how many Paths at most will be created
	\param	cost_name			The cost type for edges, represented as a C-string

	\returns					HF_STATUS::OK if a valid path is found
								HF_STATUS::NO_PATH if no valid path is found
