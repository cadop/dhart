...
Now that we have the mesh, Generating the graph is relatively straightforward. Internally, the graph uses the EmbreeRayTracer which requires a BVH, so we can follow the process as the previous tutorial to create a BVH for it.
...
Now, we are ready to generate a graph. For now, we will begin with minimum required settings, then work our way up to some other examples. To generate a graph at minimum, the following arguments are requried:

1) A BVH containing the mesh you want to use for graph generation.
2) A starting point.
3) The spacing nodes.

So let's start small and use the default settings for everything else.
```C#
