/*!
	\file		Sample_CreateVisibilityGraph.cpp
	\brief		Driver source file for testing the creation of a visibility graph
...
namespace CInterfaceTests {
	...
	*/
	...
	void create_visibility_graph(HINSTANCE dll_hf);
}
...
int main(int argc, const char* argv[]) {
	...
	*/
	...
	if (dll_DHART_API == nullptr) {
		...
		///
		CInterfaceTests::create_visibility_graph(dll_DHART_API);
...
/*!
	...
	----------------------------------
		...
		CreateVisibilityAllToAll - visibility_graph_C.h
		GenerateGraph - spatialstructures_C.h
		GetCSRPointers - spatialstructures_C.h
	...
	Required defintions:
		enum HF::SpatialStructures::NODE_TYPE {
			GRAPH = 0;
			POI = 1;
			OTHER = 2;
		};
...
*/
void CInterfaceTests::create_visibility_graph(HINSTANCE dll_hf) {
	///
	...
	typedef int (*p_GetCSRPointers)(HF::SpatialStructures::Graph*, int*, int*, int*,
		float**, int**, int**, const char*);

	typedef int (*p_CreateVisibilityGraphAllToAll)(HF::RayTracer::EmbreeRayTracer*, const float*, int,
		HF::SpatialStructures::Graph**, float);
	...
	auto CreateVisibilityGraphAllToAll = (p_CreateVisibilityGraphAllToAll)GetProcAddress(dll_hf, "CreateVisibilityGraphAllToAll");
	...
	/// Example begins here
	///
	std::cout << "\n--- Create Visibility Graph example ---\n" << std::endl;
