...
\end{DoxyItemize}
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{_mesh_from_unity_autotoc_md24}{}\doxysection{Intro}\label{_mesh_from_unity_autotoc_md24}
This tutorial will demonstrate how to pass geometry from the scene in Unity to Human Factors. We\textquotesingle{}ll be using the project created in the previous guide\+: H\+:/\+H\+F\+Gitlab/\+Analysis/docs/C\# Documentation/markdown/3\+\_\+graph\+\_\+generator.\+md \char`\"{}\+The Graph Generator\char`\"{}, and we\textquotesingle{}ll be using concepts covered in the previous guides.\hypertarget{_mesh_from_unity_autotoc_md25}{}\doxysection{Scene Setup}\label{_mesh_from_unity_autotoc_md25}
...
From here we will create a new plane then translate it directly to the origin.\hypertarget{_mesh_from_unity_autotoc_md26}{}\doxysubsection{Creating the Plane}\label{_mesh_from_unity_autotoc_md26}
Using the menubar at the top of the screen select Game\+Object $>$ 3D Object $>$ Plane.



Once clicked, a new 1x1 plane will be created, however it may not have been created at the origin exactly. For this tutorial we will reset the plane\textquotesingle{}s position just be be sure it\textquotesingle{}s always placed exactly in the same place.\hypertarget{_mesh_from_unity_autotoc_md27}{}\doxysubsection{Resetting the Plane\textquotesingle{}s Position}\label{_mesh_from_unity_autotoc_md27}
...
Now that we have the plane ready to go, we can begin working on the script to get its vertices and triangles.\hypertarget{_mesh_from_unity_autotoc_md28}{}\doxysection{Writing the Script}\label{_mesh_from_unity_autotoc_md28}
Open up the New Behavior script we created in the H\+:/\+H\+F\+Gitlab/\+Analysis/docs/C\# Documentation/markdown/3\+\_\+graph\+\_\+generator.\+md \char`\"{}previous tutorial\char`\"{}\hypertarget{_mesh_from_unity_autotoc_md29}{}\doxysubsection{Set Usings}\label{_mesh_from_unity_autotoc_md29}
...
\hypertarget{_mesh_from_unity_autotoc_md30}{}\doxysubsection{Setup for adding references through the unity inspector}\label{_mesh_from_unity_autotoc_md30}
...
Later we\textquotesingle{}ll use the unity inspector to assign the plane we created in the scene to this object\hypertarget{_mesh_from_unity_autotoc_md31}{}\doxysubsection{Passing Meshes from Game\+Objects to Human\+Factors}\label{_mesh_from_unity_autotoc_md31}
Now that we have a reference to the plane game object we want to use, we need to get the raw vertices and faces of plane so we can pass it to Human\+Factors for use in the graph generator.\hypertarget{_mesh_from_unity_autotoc_md32}{}\doxysubsubsection{Getting a reference to the mesh held by a specific Game Object}\label{_mesh_from_unity_autotoc_md32}
...
\end{DoxyCode}
\hypertarget{_mesh_from_unity_autotoc_md33}{}\doxysubsubsection{Getting the vertices and triangles from a Unity Mesh}\label{_mesh_from_unity_autotoc_md33}
...
\hypertarget{_mesh_from_unity_autotoc_md34}{}\doxysubsubsection{Transforming the Mesh from Y-\/\+Up to Z-\/\+Up}\label{_mesh_from_unity_autotoc_md34}
...
screenshot, so instead I\textquotesingle{}ll just post the full code in this document here.\hypertarget{_mesh_from_unity_autotoc_md35}{}\doxysection{Executing the Script}\label{_mesh_from_unity_autotoc_md35}
\hypertarget{_mesh_from_unity_autotoc_md36}{}\doxysubsection{Adding References Through the Unity Inspector}\label{_mesh_from_unity_autotoc_md36}
\hypertarget{_mesh_from_unity_autotoc_md37}{}\doxysubsection{Comparing Output}\label{_mesh_from_unity_autotoc_md37}
