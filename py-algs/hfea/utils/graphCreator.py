from . import geomUtils as gt
from . import rayUtils as ru
...
def buildGraph(To_Do,graphParams, geometry_bvh,graph): 
    ...
    def computeParent(parent):
        ...
        # if the number of valid children is less than the required minimum (but not empty)
        if rel_count < graphParams['min_connection']:
            weight = [0,0,0,0]
            relations = [] # reset relations to none
            relations.append( [parent,parent,[999999999999.9999 for x in weight] ] )
