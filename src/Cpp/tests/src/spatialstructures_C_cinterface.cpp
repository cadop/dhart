...
namespace CInterfaceTests {
	...
	TEST(_spatialstructures_cinterface, AddNodeAttributesFloat) {
		// Status code variable, value returned by C Interface functions
		// See documentation for HF::Exceptions::HF_STATUS for error code definitions.
		int status = 0;
		...
		//! [snippet_spatialstructuresC_AddNodeAttributesFloat]
		// We do not need edge cost type now.
		const char* cost_type = "";
	...
	TEST(_spatialstructures_cinterface, GetNodeAttributesFloat) {
		...
		// Allocate an array of char arrays to meet the preconditions of GetNodeAttributesFloat
		float* scores_out = new float [g.size()];
		int scores_out_size = 0;

		// By the postconditions of GetNodeAttributesFloat, this should update scores_out,
		// and scores_out_size with the variables we need
	...
	TEST(_spatialstructures_cinterface, GetNodeAttributesByID) {
		...
		// Define a list of the node IDs we want attributes for
		std::vector<int> subset_ids{ 1, 3 };

		// Define the output array and an integer to store the actual output size 
		char** scores_out = new char* [subset_ids.size()];
		int scores_out_size = 0;

		// Get node attrbutes for the IDs
		GetNodeAttributesByID(&g, subset_ids.data(), attr_type.c_str(), subset_ids.size(), scores_out, &scores_out_size);
	...
	TEST(_spatialstructures_cinterface, GetNodeAttributesByIDFloat) {
		...
		std::vector<int> ids{ 1, 3, 5, 7 };
		std::string attr_type = "test attribute";
		const float scores[4] = { 29.3, 10.7, 3.5, 18.6 };
		...
		// Define the output array and an integer to store the actual output size 
		float* scores_out = new float[subset_ids.size()];
		int scores_out_size = 0;

		// Get node attrbutes for the IDs
		GetNodeAttributesByIDFloat(&g, subset_ids.data(), attr_type.c_str(), subset_ids.size(), scores_out, &scores_out_size);
