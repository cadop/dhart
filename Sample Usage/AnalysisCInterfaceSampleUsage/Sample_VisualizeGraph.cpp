...
void CInterfaceTests::visualize_graph(HINSTANCE dll_hf) {
	...
	//
	// The graph can be plotted in 3D for illustrative purposes
	// (although it is better to load an obj or use a proper engine like OpenGL.)
	//
	/*
		# Plot the graph in 3D
		fig = plt.figure()
	...
	*/
	//
	// TODO C++, may just do a 2D ASCII-art example instead?
	//
	...
	*/
	...
	//
	// To illustrate the thresholds of this wavy hill, we can reduce the slope tolerance
	// and plot a new graph which shows certain areas are not reachable.
	//
	/*
